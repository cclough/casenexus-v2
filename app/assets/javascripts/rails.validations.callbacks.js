...
clientSideValidations.callbacks.element.fail = function(element, message, callback) {
  ...
  if (element.data('valid') !== false) {
  	element.parent().parent().find('.message').popover('show');
...
clientSideValidations.callbacks.element.pass = function(element, callback) {
  ...
  element.parent().parent().find('.message').popover('hide');
  element.parent().find('.message').effect('fade', {}, 2000, callback);
  element.animate( { backgroundColor: "#ffffff" }, 1000, callback);
}

// title="A Title" data-content="And here's some amazing content. It's very engaging. right?"